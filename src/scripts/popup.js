...
function initApp({ops}){
  ...
  document.querySelector('#file-input').addEventListener('FilePond:processfile', e => {
    ...
    $info.find('.filepond--file-info-main').replaceWith($(`<a class="indigo lighten-5" href="${serverId}" target="_blank">${serverId}</a>`));
...
ext.tabs.query({active: true, currentWindow: true}, function(tabs) {
  ...
  chrome.tabs.sendMessage(activeTab.id, { action: 'process-page' }, initPageShow);
});
